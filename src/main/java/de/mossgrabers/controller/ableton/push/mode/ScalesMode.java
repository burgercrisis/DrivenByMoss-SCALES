...
public class ScalesMode extends BaseMode<IItem> {
    ...
    public void onKnobValue(final int index, final int value) {
        ...
        this.scales.changeAllScale(value);
        this.update();
    }
    ...
    public void onFirstRow(final int index, final ButtonEvent event) {
        ...
        if (index == 0) {
            if (this.isPushModern)
                this.scales.nextAllScale();
            else
                this.scales.prevAllScale();
    ...
    public int getButtonColor(final ButtonID buttonID) {
        ...
        if (index >= 0) {
            ...
            return this.colorManager.getColorIndex(
                    offset == index - 1 ? AbstractMode.BUTTON_COLOR_HI : AbstractFeatureGroup.BUTTON_COLOR_ON);
        }
        ...
        if (index >= 0) {
            ...
            final int offset = this.scales.getScaleOffsetIndex();
            return this.colorManager.getColorIndex(
                    offset == index - 1 + 6 ? AbstractMode.BUTTON_COLOR2_HI : AbstractMode.BUTTON_COLOR2_ON);
        }
    ...
    public void updateDisplay1(final ITextDisplay display) {
        final String[] allScaleNames = this.scales.getAllScaleNames();
        final String currentScaleName = this.scales.getCurrentScaleName();
        int selIndex = 0;
        for (int i = 0; i < allScaleNames.length; i++) {
            if (allScaleNames[i].equals(currentScaleName)) {
                selIndex = i;
                break;
            }
        }
        ...
        int pos = 0;
        for (final Pair<String, Boolean> p : Push1Display.createMenuList(4, allScaleNames, selIndex)) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            display.setCell(3, i + 1,
                    "  " + (offset == 6 + i ? Push1Display.SELECT_ARROW : " ") + Scales.BASES.get(6 + i));
        }
    ...
    public void updateDisplay2(final IGraphicDisplay display) {
        ...
        display.addListElement(6, allScaleNames, selIndex);

        final int offset = this.scales.getScaleOffsetIndex();
        final String rangeText = this.scales.getRangeText();
        for (int i = 0; i < 6; i++)
            display.addOptionElement(i == 3 ? "Note range: " + rangeText : "", Scales.BASES.get(6 + i), offset == 6 + i,
                    "",
                    Scales.BASES.get(i), offset == i, false);

        display.addOptionElement("", this.scales.isChromatic() ? "Chromatc" : "In Key", this.scales.isChromatic(), "",
                "",
                false, false);
    }
